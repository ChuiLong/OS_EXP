...
obj/__user_stride_test.out:     file format elf64-littleriscv
...
exit(int error_code) {
  ...
  8000de:	55e50513          	addi	a0,a0,1374 # 800638 <main+0x12e>
...
printnum(void (*putch)(int, void*), void *putdat,
  ...
  80013e:	51678793          	addi	a5,a5,1302 # 800650 <main+0x146>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
  ...
  80019a:	4eea8a93          	addi	s5,s5,1262 # 800684 <main+0x17a>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
  ...
  8001a2:	702b8b93          	addi	s7,s7,1794 # 8008a0 <error_string>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
  ...
  8003f8:	28c60613          	addi	a2,a2,652 # 800680 <main+0x176>
  ...
  80040a:	26a60613          	addi	a2,a2,618 # 800670 <main+0x166>
  ...
  80041c:	25040413          	addi	s0,s0,592 # 800668 <main+0x15e>
  ...
  8004a6:	1c640413          	addi	s0,s0,454 # 800668 <main+0x15e>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
  ...
  8004bc:	1b1a0a13          	addi	s4,s4,433 # 800669 <main+0x15f>
...
int main(void) {
  ...
  800510:	45c50513          	addi	a0,a0,1116 # 800968 <error_string+0xc8>
int main(void) {
  800514:	f486                	sd	ra,104(sp)
  800516:	eca6                	sd	s1,88(sp)
  800518:	e8ca                	sd	s2,80(sp)
  80051a:	e4ce                	sd	s3,72(sp)
  80051c:	e0d2                	sd	s4,64(sp)
  80051e:	fc56                	sd	s5,56(sp)
  800520:	f0a2                	sd	s0,96(sp)
    cprintf("Stride Scheduler Test\n");
  800522:	b1fff0ef          	jal	ra,800040 <cprintf>
    cprintf("=====================\n");
  800526:	00000517          	auipc	a0,0x0
  80052a:	45a50513          	addi	a0,a0,1114 # 800980 <error_string+0xe0>
  ...
  800538:	46450513          	addi	a0,a0,1124 # 800998 <error_string+0xf8>
  80053c:	b05ff0ef          	jal	ra,800040 <cprintf>
    
    // 设置父进程优先级最高，避免干扰
    lab6_setpriority(TOTAL + 1);
  800540:	4519                	li	a0,6
  800542:	890a                	mv	s2,sp
  800544:	babff0ef          	jal	ra,8000ee <lab6_setpriority>
  800548:	89ca                	mv	s3,s2
    
    for (i = 0; i < TOTAL; i++) {
  80054a:	4481                	li	s1,0
  80054c:	00001a17          	auipc	s4,0x1
  800550:	ab4a0a13          	addi	s4,s4,-1356 # 801000 <count>
  800554:	4a95                	li	s5,5
        count[i] = 0;
  800556:	00249793          	slli	a5,s1,0x2
  80055a:	97d2                	add	a5,a5,s4
  80055c:	0007a023          	sw	zero,0(a5)
        if ((pids[i] = fork()) == 0) {
  800560:	b89ff0ef          	jal	ra,8000e8 <fork>
  800564:	00a9a023          	sw	a0,0(s3)
  800568:	842a                	mv	s0,a0
  80056a:	c551                	beqz	a0,8005f6 <main+0xec>
            }
        ...
        if (pids[i] < 0) {
  80056c:	06054d63          	bltz	a0,8005e6 <main+0xdc>
    for (i = 0; i < TOTAL; i++) {
  800570:	2485                	addiw	s1,s1,1
  800572:	0991                	addi	s3,s3,4
  800574:	ff5491e3          	bne	s1,s5,800556 <main+0x4c>
            cprintf("fork failed!\n");
            return -1;
        }
    }
  ...
  80057c:	48850513          	addi	a0,a0,1160 # 800a00 <error_string+0x160>
  800580:	ac1ff0ef          	jal	ra,800040 <cprintf>
    
    // 等待所有子进程结束
    for (i = 0; i < TOTAL; i++) {
  800584:	0820                	addi	s0,sp,24
  800586:	02c10993          	addi	s3,sp,44
        waitpid(pids[i], &status[i]);
        cprintf("Child with priority %d exited\n", status[i]);
  80058a:	00000497          	auipc	s1,0x0
  80058e:	4a648493          	addi	s1,s1,1190 # 800a30 <error_string+0x190>
        waitpid(pids[i], &status[i]);
  800592:	00092503          	lw	a0,0(s2)
  800596:	85a2                	mv	a1,s0
    for (i = 0; i < TOTAL; i++) {
  800598:	0411                	addi	s0,s0,4
        waitpid(pids[i], &status[i]);
  80059a:	b51ff0ef          	jal	ra,8000ea <waitpid>
        cprintf("Child with priority %d exited\n", status[i]);
  80059e:	ffc42583          	lw	a1,-4(s0)
  8005a2:	8526                	mv	a0,s1
    for (i = 0; i < TOTAL; i++) {
  8005a4:	0911                	addi	s2,s2,4
        cprintf("Child with priority %d exited\n", status[i]);
  8005a6:	a9bff0ef          	jal	ra,800040 <cprintf>
    for (i = 0; i < TOTAL; i++) {
  8005aa:	ff3414e3          	bne	s0,s3,800592 <main+0x88>
    }
    
    cprintf("\n=====================\n");
  8005ae:	00000517          	auipc	a0,0x0
  8005b2:	4a250513          	addi	a0,a0,1186 # 800a50 <error_string+0x1b0>
  8005b6:	a8bff0ef          	jal	ra,800040 <cprintf>
    cprintf("Stride Scheduler Test PASSED!\n");
  8005ba:	00000517          	auipc	a0,0x0
  8005be:	4ae50513          	addi	a0,a0,1198 # 800a68 <error_string+0x1c8>
  8005c2:	a7fff0ef          	jal	ra,800040 <cprintf>
    cprintf("All children completed successfully.\n");
  8005c6:	00000517          	auipc	a0,0x0
  8005ca:	4c250513          	addi	a0,a0,1218 # 800a88 <error_string+0x1e8>
  8005ce:	a73ff0ef          	jal	ra,800040 <cprintf>
    
    return 0;
  8005d2:	4501                	li	a0,0
}
  8005d4:	70a6                	ld	ra,104(sp)
  8005d6:	7406                	ld	s0,96(sp)
  8005d8:	64e6                	ld	s1,88(sp)
  8005da:	6946                	ld	s2,80(sp)
  8005dc:	69a6                	ld	s3,72(sp)
  8005de:	6a06                	ld	s4,64(sp)
  8005e0:	7ae2                	ld	s5,56(sp)
  8005e2:	6165                	addi	sp,sp,112
  8005e4:	8082                	ret
            cprintf("fork failed!\n");
  8005e6:	00000517          	auipc	a0,0x0
  8005ea:	40a50513          	addi	a0,a0,1034 # 8009f0 <error_string+0x150>
  8005ee:	a53ff0ef          	jal	ra,800040 <cprintf>
            return -1;
  8005f2:	557d                	li	a0,-1
  8005f4:	b7c5                	j	8005d4 <main+0xca>
            lab6_setpriority(i + 1);  // 优先级分别为1,2,3,4,5
  8005f6:	0014899b          	addiw	s3,s1,1
  8005fa:	854e                	mv	a0,s3
  8005fc:	af3ff0ef          	jal	ra,8000ee <lab6_setpriority>
            cprintf("Child %d: pid=%d, priority=%d\n", i, getpid(), i + 1);
  800600:	aedff0ef          	jal	ra,8000ec <getpid>
  800604:	862a                	mv	a2,a0
  800606:	86ce                	mv	a3,s3
  800608:	85a6                	mv	a1,s1
  80060a:	00000517          	auipc	a0,0x0
  80060e:	3c650513          	addi	a0,a0,966 # 8009d0 <error_string+0x130>
  800612:	a2fff0ef          	jal	ra,800040 <cprintf>
            for (j = 0; j < LOOP_COUNT * (i + 1); j++) {
  800616:	6789                	lui	a5,0x2
  800618:	7107879b          	addiw	a5,a5,1808
  80061c:	02f9893b          	mulw	s2,s3,a5
                volatile int k = j * j;
  800620:	028407bb          	mulw	a5,s0,s0
            for (j = 0; j < LOOP_COUNT * (i + 1); j++) {
  800624:	2405                	addiw	s0,s0,1
                volatile int k = j * j;
  800626:	cc3e                	sw	a5,24(sp)
                (void)k;
  800628:	47e2                	lw	a5,24(sp)
            for (j = 0; j < LOOP_COUNT * (i + 1); j++) {
  80062a:	fe891be3          	bne	s2,s0,800620 <main+0x116>
            exit(i + 1);  // 返回优先级作为退出码
  80062e:	854e                	mv	a0,s3
  800630:	aa3ff0ef          	jal	ra,8000d2 <exit>
