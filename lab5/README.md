...
下面结合 **lab5（用户程序 / 用户进程）实验内容**，从“实验中的重要知识点 ↔ OS 原理中的知识点”以及“OS 原理中重要但实验未覆盖的知识点”两个角度进行系统总结与对比说明。
## 一、实验中重要的知识点 与 OS 原理对应关系

| 实验中的知识点                  | 对应的 OS 原理知识点          | 实验中的含义                               | OS 原理中的含义                         | 二者关系 / 差异理解                        |
| ------------------------ | --------------------- | ------------------------------------ | --------------------------------- | ---------------------------------- |
| 用户进程（User Process）       | 进程（Process）抽象         | 实验中实现的是**用户态进程**，有独立地址空间，通过系统调用与内核交互 | OS 原理中进程是资源分配和调度的基本单位，包含代码、数据、PCB | 实验是原理的**具体化与简化实现**，强调“用户态 / 内核态隔离” |
| 进程控制块（PCB / proc_struct） | 进程控制块 PCB             | 实验中用结构体保存进程状态、寄存器上下文、页表等             | 原理中 PCB 是进程存在的唯一标志                | 实验让抽象 PCB **落地为真实数据结构**            |
| 创建用户进程                   | 进程创建（fork / exec 思想）  | 实验中通过加载 ELF、初始化 trapframe 创建用户进程     | 原理中 fork/exec 用于创建并装载新程序          | 实验通常**合并/简化 fork + exec**，但思想一致    |
| ELF 程序加载                 | 程序装载（Program Loading） | 实验中解析 ELF，将代码/数据映射进用户地址空间            | 原理中描述从磁盘到内存的装载过程                  | 实验是 OS 原理中“装载器”的一个教学级实现            |
| 用户态 / 内核态切换              | 特权级机制                 | 通过 trap 从用户态进入内核态                    | 原理中利用 CPU 特权级保护 OS                | 实验强调**机制层面**，原理强调**设计动机与安全性**      |
| Trap / 异常处理              | 异常与中断机制               | 系统调用、非法访问都会触发 trap                   | 原理中异常是 CPU 主动进入内核的方式              | 实验是原理中“异常控制流”的直接体现                 |
| 系统调用（syscall）            | 系统调用接口                | 用户程序通过 syscall 请求内核服务                | 原理中系统调用是用户态访问内核的唯一合法途径            | 实验实现了 syscall 的**完整调用链路**          |
| Trapframe                | 上下文保存与恢复              | 实验中 trapframe 保存用户寄存器现场              | 原理中上下文切换需保存 CPU 状态                | 实验体现了“上下文”的**硬件 + 软件协作**           |
| 进程退出（exit）               | 进程生命周期管理              | 实验中回收进程资源，设置退出状态                     | 原理中 exit 释放资源并通知父进程               | 实验是原理中生命周期“终止阶段”的实现                |
| 内核调度用户进程                 | 进程调度                  | 实验中让用户进程获得 CPU 执行                    | 原理中调度决定哪个进程运行                     | 实验中的调度较简单，**未强调复杂算法**              |
| 用户地址空间                   | 虚拟内存                  | 每个用户进程有独立虚拟地址空间                      | 原理中虚拟内存提供隔离与抽象                    | 实验重点在“隔离正确性”，而非性能优化                |

---

## 二、实验中出现但**原理中弱化 / 未系统展开**的知识点

| 实验知识点             | 是否有明确原理对应 | 说明                         |
| ----------------- | --------- | -------------------------- |
| RISC-V Trap 入口汇编  | 部分对应      | 原理中讲“异常机制”，但不强调具体 ISA 汇编细节 |
| trapframe 的具体字段布局 | 弱对应       | 原理关注“保存上下文”，实验关注“怎么存”      |
| OpenSBI 参与启动      | 原理中较少涉及   | 属于实验平台相关内容，不是 OS 核心理论      |
| ELF 解析代码细节        | 抽象化处理     | 原理中讲“装载”，不关心 ELF 字段实现      |

---

## 三、OS 原理中**非常重要，但实验中没有直接对应实现**的知识点

| OS 原理中的重要知识点           | 实验中是否涉及 | 原因与理解                |
| ---------------------- | ------- | -------------------- |
| 进程调度算法（RR / CFS / 优先级） | ❌       | 实验只需“能跑”，不追求调度公平性与性能 |
| 进程同步与互斥（锁 / 信号量）       | ❌       | 用户进程较少，未涉及并发冲突       |
| 进程间通信（IPC）             | ❌       | 实验不涉及多进程协作           |
| 文件系统                   | ❌       | 用户程序不从文件系统加载         |
| 虚拟内存置换（LRU / FIFO）     | ❌       | 实验无内存压力场景            |
| 缺页异常（Page Fault）       | ❌       | 实验内存映射一次完成           |
| 多核并发与 SMP              | ❌       | 实验运行在单核环境            |
| 安全与权限模型                | ❌       | 实验仅体现“用户/内核”二级权限     |
| 信号（Signal）             | ❌       | 属于更高层的进程控制机制         |
| 父子进程关系、wait            | ❌       | 实验中多为孤立用户进程          |
