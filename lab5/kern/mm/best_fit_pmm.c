...
best_fit_alloc_pages(size_t n) {
    ...
    while ((le = list_next(le)) != &free_list) {
        struct Page *p = le2page(le, page_link);
        if (p->property >= n) {  // 首先确保空闲块大小满足需求
            if (page == NULL || p->property < page->property) {  // 在所有满足条件的块中找最小的
                page = p;
            }
        }
    }
...
best_fit_free_pages(struct Page *base, size_t n) {
    ...
    base->property = n;        // 设置当前页块的大小为n
    SetPageProperty(base);     // 标记这个页面是一个空闲块的首页面
    nr_free += n;             // 增加系统中空闲页面的总数
    ...
    if (le != &free_list) {
        ...
        if (p + p->property == base) {  // 判断前一个块是否与当前块物理地址连续
